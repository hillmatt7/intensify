...
class TestIntegrateVecAllKernels:
    ...
    def test_exponential(self):
        ...
        self._check(
            ExponentialKernel(0.3, 1.5), bt.array([0.01, 0.1, 0.5, 2.0, 10.0, 100.0])
        )
    ...
    def test_sum_exponential(self):
        ...
        self._check(
            SumExponentialKernel([0.2, 0.1], [1.0, 5.0]),
            bt.array([0.01, 0.5, 2.0, 10.0]),
        )
    ...
    def test_power_law(self):
        ...
        self._check(
            PowerLawKernel(0.4, 0.7, 0.2), bt.array([0.01, 0.1, 1.0, 5.0, 20.0])
        )
    ...
    def test_approx_power_law(self):
        ...
        self._check(
            ApproxPowerLawKernel(0.3, 0.5, 0.1, r=2.0, n_components=6),
            bt.array([0.05, 0.5, 2.0]),
        )
...
class TestVectorizedMultivariateLLDemo:
    def test_bivariate_ll_finite(self):
        ...
        events = [
            bt.array([0.5, 1.5, 3.0, 5.0, 7.0]),
            bt.array([0.8, 2.0, 4.0, 6.5, 9.0]),
        ]
...
class TestJitNegLLFactories:
    ...
    def test_sum_exponential(self):
        ...
        ej = jnp.asarray(np.asarray(EVENTS_MEDIUM, dtype=float), dtype=jnp.float64)
        neg_ll = _make_jit_neg_loglik_sum_exp(
            ej, jnp.asarray(T_MEDIUM, dtype=jnp.float64), 2
        )
    ...
    def test_power_law(self):
        ...
        neg_ll = _make_jit_neg_loglik_power_law(
            ej, jnp.asarray(T_SMALL, dtype=jnp.float64)
        )
    ...
    def test_approx_power_law(self):
        ...
        ej = jnp.asarray(np.asarray(EVENTS_SMALL, dtype=float), dtype=jnp.float64)
        neg_ll = _make_jit_neg_loglik_approx_pl(
            ej, jnp.asarray(T_SMALL, dtype=jnp.float64), 1.5, 5
        )
...
class TestJaxGradVsFiniteDiff:
    def test_exp_grad_matches_fd(self):
        ...
        fd_grad = np.zeros(3)
        for i in range(3):
            xp = np.asarray(x0).copy()
            xp[i] += eps
            xm = np.asarray(x0).copy()
            xm[i] -= eps
            fd_grad[i] = (
                float(neg_ll(jnp.array(xp))) - float(neg_ll(jnp.array(xm)))
            ) / (2 * eps)
...
class TestJaxHessianDemo:
    def test_std_errors_at_optimum(self):
        ...
        res = spo.minimize(
            ...
            method="L-BFGS-B",
            jac=True,
            bounds=[(1e-8, None), (1e-8, 0.999), (1e-8, None)],
        )
        ...
        for name in names:
            assert se[name] >= 0, (
                f"SE for {name} should be non-negative, got {se[name]}"
            )
            assert se[name] < 10.0, f"SE for {name} unreasonably large: {se[name]}"
        assert any(se[n] > 0 for n in names), (
            "At least one SE should be strictly positive"
        )
...
class TestFitJaxEndToEnd:
    def test_exponential_recovery(self):
        ...
        assert result.convergence_info["success"] is True, result.convergence_info[
            "message"
        ]
    ...
    def test_sum_exponential(self):
        ...
        result = MLEInference(max_iter=500).fit(
            UnivariateHawkes(
                mu=0.4, kernel=SumExponentialKernel([0.15, 0.1], [1.0, 3.0])
            ),
            EVENTS_MEDIUM,
            T=T_MEDIUM,
        )
    ...
    def test_approx_power_law(self):
        ...
        result = MLEInference(max_iter=500).fit(
            UnivariateHawkes(
                mu=0.4,
                kernel=ApproxPowerLawKernel(0.3, 0.5, 0.1, r=1.5, n_components=5),
            ),
            EVENTS_MEDIUM,
            T=T_MEDIUM,
        )
        # Phase 3 port: ApproxPowerLawKernel routes through Rust.
        assert result.convergence_info["backend"] == "rust"
...
class TestJaxWhileSimulation:
    ...
    def test_high_rate_simulation(self):
        ...
        assert len(events) > 10, (
            f"High-rate process should produce many events, got {len(events)}"
        )
...
class TestMultivariateMLE:
    def test_bivariate_fit(self):
        ...
        mv = MultivariateHawkes(n_dims=2, mu=[0.3, 0.4], kernel=kernels)
        events = [
            bt.array([0.5, 1.5, 3.0, 5.0, 7.0, 8.5]),
            bt.array([0.8, 2.0, 4.0, 6.5, 9.0]),
        ]
...
class TestRecursiveVsGeneralAfterRefactor:
    ...
    def test_approx_pl_recursive_vs_general(self):
        ...
        proc = UnivariateHawkes(
            mu=0.4, kernel=ApproxPowerLawKernel(0.3, 0.5, 0.1, r=1.5, n_components=5)
        )
        rec = _recursive_likelihood_numpy(proc, EVENTS_SMALL, T_SMALL)
        gen = _general_likelihood_numpy(proc, EVENTS_SMALL, T_SMALL)
        np.testing.assert_allclose(rec, gen, atol=1e-8)
...
class TestSimulateFitRoundTrip:
    ...
    def test_exponential_round_trip(self):
        ...
        proc_true = UnivariateHawkes(
            mu=true_mu, kernel=ExponentialKernel(true_a, true_b)
        )
        ...
        assert abs(est_mu - true_mu) / true_mu < 0.6, (
            f"mu recovery: {est_mu} vs {true_mu}"
        )
        assert abs(est_a - true_a) / true_a < 0.6, (
            f"alpha recovery: {est_a} vs {true_a}"
        )
