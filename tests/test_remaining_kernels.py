...
def test_power_law_nonstationary_warning():
    pytest.skip(
        "PowerLawKernel does not emit an init warning; stationarity checked via is_stationary()."
    )


def test_approx_power_law_evaluate():
    kernel = ApproxPowerLawKernel(
        alpha=1.0, beta_pow=0.8, beta_min=0.1, r=1.5, n_components=5
    )
...
def test_approx_power_law_l1_norm():
    kernel = ApproxPowerLawKernel(
        alpha=0.7, beta_pow=0.6, beta_min=0.2, n_components=10
    )
